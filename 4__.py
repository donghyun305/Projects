...
class TrainingModel(object):
    ...
    def train_svm_for_noisy_data(self, rho_plus, rho_minus, alpha):
        X_train = [(x, y) for _, x, y in self.noised_train_set]
        y_train = [label for label, _, _ in self.noised_train_set]
        self.clf = train_model(X_train, y_train, rho_plus, rho_minus, alpha)
...
def train_model(X_train, y_train, rho_plus, rho_minus, alpha):
    ...
    # Create the SVM with custom kernel
    svm_custom = SVC(kernel=lambda X, Y: custom_kernel(X, Y, rho_plus, rho_minus, alpha))

    # Create the GridSearchCV object
    grid_search = GridSearchCV(svm_custom, param_grid=param_grid, cv=cv)
...
def custom_kernel(X, Y, rho_plus, rho_minus, alpha):
    # Compute the linear kernel
    linear_kernel = np.dot(X, Y.T)

    # Compute the noise kernel
    noise_kernel = rho_plus * np.dot(X, X.T) + rho_minus * np.dot(Y, Y.T)

    # Combine the kernels using the given alpha value
    combined_kernel = alpha * linear_kernel + (1 - alpha) * noise_kernel
...
if __name__ == "__main__":
    ...
    alpha = 0.5
    model.train_svm_for_noisy_data(rho_p, rho_n, alpha)
    noisy_accuracy = model.test_accuracy()
    print("Noisy data test accuracy (with custom model): {:.2f}%".format(noisy_accuracy * 100))

    noisy_cv_score = model.cross_val_score(k, kernel='linear', C=1.0)
    print("Noisy data cross-validation score (with custom model) ({}-fold): {:.2f}%".format(k, noisy_cv_score * 100))

    # 코드 생략...

    print("Noisy data decision boundary (with custom model):")
    model.plot_decision_boundary()
